...
// point hessian takes some of the information from immature point to estimate the idepth.
    PointHessian::PointHessian(const ImmaturePoint *const rawPoint, CalibHessian *Hcalib) {
        instanceCounter++;
        // take the host frame from the immature point.
        host = rawPoint->host;
        hasDepthPrior = false;
        ...
        // take the u, v index from image plane
        u = rawPoint->u;
        v = rawPoint->v;
        // if the idepth is NAN the point hessian will be discard.
        assert(std::isfinite(rawPoint->idepth_max));
        // they never maintained the idepth_GT term, I think if they want to use this GT value
    ...
    // set nullspace for scale pose and affine model.
