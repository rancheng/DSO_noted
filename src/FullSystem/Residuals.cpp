...
// this function linearized the residual jacobian
    double PointFrameResidual::linearize(CalibHessian *HCalib) {
        ...
        float d_d_x, d_d_y; // depth partial with respect to x and y
        {
            ...
            centerProjectedTo = Vec3f(Ku, Kv,
                                      new_idepth); // Ku, Kv should be the projected points, together with new_idepth will be the point in the center of residual pattern.
        ...
        for (int idx = 0; idx <
                          patternNum; idx++) // 8 dimensional residual pattern point's partial, plus 1 center point, which is 9 iterations
        {
            float Ku, Kv;
            ...
            float residual =
                    hitColor[0] - (float) (affLL[0] * color[idx] + affLL[1]); // calculate photometric error here
