 ...
 */
    ...
    public:
        ...
        inline void updateSSE(
                ...
                const __m128 J6, const __m128 J7,
                const __m128 J8) {
            // this update 4*45 times with is 9+8+7+...+1
            // which is basically the upper triangle part of the matrix J'*J
            float *pt = SSEData;
            _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt), _mm_mul_ps(J0, J0)));
            pt += 4;
            _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt), _mm_mul_ps(J0, J1)));
    ...
    private:
        EIGEN_ALIGN16 float SSEData[4 * 45]; //SSEData stores 4 by 45 matrix
        EIGEN_ALIGN16 float SSEData1k[4 * 45];
        EIGEN_ALIGN16 float SSEData1m[4 * 45];
        float numIn1, numIn1k, numIn1m; // counter that count the number of data refence in SSEData family above.

        // this function aggregate all the SSEData into 1k and 1m scale.
        void shiftUp(bool force) {
            if (numIn1 > 1000 || force) {
                // reference: https://www.cs.uaf.edu/2009/fall/cs301/lecture/11_13_sse_intrinsics.html
                // here they store results of  _mm_add_ps(_mm_load_ps(SSEData+4*i),_mm_load_ps(SSEData1k+4*i))
                // into address: SSEData1k+4*i
                // _mm_add_ps add the _mm_load_ps(SSEData+4*i) and ,_mm_load_ps(SSEData1k+4*i)
                // so this step is add every 4 float in SSEData into SSEData1k.
                // _mm_load_ps(float* src): Load 4 floats from a 16-byte aligned address
                for (int i = 0; i < 45; i++)
                    _mm_store_ps(SSEData1k + 4 * i, _mm_add_ps(_mm_load_ps(SSEData + 4 * i), _mm_load_ps(
                            SSEData1k + 4 * i))); // Store 4 floats to an aligned address.
                numIn1k += numIn1; // after one aggregation, counter add 1 for the SSEData1k
                numIn1 = 0; // this is a count for SSEData. since SSEData will be clear out next step, this set SSEData refence to 0.
                memset(SSEData, 0, sizeof(float) * 4 * 45); // set all 4*45 size SSEData memory slots as 0
            }
            // similar to the code block above, they aggregate the SSEData1k into SSEData1m
            // 1k means 1000, 1m means 1 million
            if (numIn1k > 1000 || force) {
                for (int i = 0; i < 45; i++)
                    _mm_store_ps(SSEData1m + 4 * i,
                                 _mm_add_ps(_mm_load_ps(SSEData1k + 4 * i), _mm_load_ps(SSEData1m + 4 * i)));
                ...
                numIn1k = 0;
                memset(SSEData1k, 0,
                       sizeof(float) * 4 * 45); // aggregate the data1k to data1m and clear out data1k content.
            }
        }
    };
}
